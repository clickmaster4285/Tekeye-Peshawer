...
from .module_catalog import normalize_allowed_modules
...
class UserCreateSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        if instance.role == GLOBAL_ADMIN_ROLE:
            ...
            data["allowed_modules"] = list(instance.allowed_modules or [])
        return data

    def validate_allowed_modules(self, value):
        return normalize_allowed_modules(value)
    ...
    def validate(self, attrs):
        ...
        # Changing role clears custom module grants so the new role's default sidebar applies,
        # unless Super Admin is also sending allowed_modules in the same request.
        if (
            self.instance is not None
            and "role" in attrs
            and attrs.get("role") != self.instance.role
            and "allowed_modules" not in attrs
        ):
            attrs["allowed_modules"] = []
        ...
        if actor and getattr(actor, "is_authenticated", False):
            ...
            if "allowed_modules" in attrs and not is_global_admin(actor):
                # Location admin / HR may still clear modules via role change above;
                # only block non-admins from explicitly setting grants.
                if attrs.get("allowed_modules"):
                    raise serializers.ValidationError(
                        {"allowed_modules": "Only Super Admin can change module permissions."}
                    )
            if (
                "allowed_modules" in attrs
                and self.instance is not None
                and (
                    self.instance.role == GLOBAL_ADMIN_ROLE
                    or attrs.get("role") == GLOBAL_ADMIN_ROLE
                )
            ):
                attrs["allowed_modules"] = []
    ...
    def create(self, validated_data):
        ...
        if "allowed_modules" in validated_data and not is_global_admin(
            getattr(self.context.get("request"), "user", None)
        ):
            validated_data.pop("allowed_modules", None)
    ...
    def update(self, instance, validated_data):
        ...
        if "allowed_modules" in validated_data:
            if not is_global_admin(actor):
                validated_data.pop("allowed_modules")
            elif instance.role == GLOBAL_ADMIN_ROLE:
