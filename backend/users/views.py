...
from .module_catalog import SIDEBAR_MODULES
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    @action(
        detail=False,
        methods=["get"],
        url_path="modules/catalog",
        permission_classes=[IsGlobalAdmin],
    )
    def modules_catalog(self, request):
        """Sidebar modules Super Admin can grant to users."""
        return Response({"modules": SIDEBAR_MODULES})
